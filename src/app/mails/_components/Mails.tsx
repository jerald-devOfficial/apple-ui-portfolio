...
const Mails = () => {
  ...
  const [optimisticMails, applyOptimistic] = useOptimistic(
    data,
    (state, action: MailOptimisticAction) => {
  ...
  const selectedMail =
    optimisticMails.find((mail) => mail._id === selectedMailId) ?? null

  const handleMailClick = (mailId: string) => {
    const mailFind = optimisticMails.find((mail) => mail._id === mailId)
    if (!mailFind) return

    setSelectedMailId(mailId)

    if (mailFind.read || isPending) return

    startTransition(async () => {
      applyOptimistic({ type: 'read', id: mailId })
      ...
      try {
        ...
        if (!res.ok) {
          throw new Error(`Failed to mark mail as read. Status: ${res.status}`)
        }

        await mutate()
      } catch (mailError) {
        console.error('Error marking mail as read:', mailError)
        await mutate()
      }
    })
  }
  ...
  const handleMarkAsUnread = (mailId: string) => {
    startTransition(async () => {
      applyOptimistic({ type: 'unread', id: mailId })
      ...
      try {
        ...
        if (!res.ok) {
          throw new Error(
            `Failed to mark mail as unread. Status: ${res.status}`
          )
        }

        await mutate()
      } catch (mailError) {
        console.error('Error marking mail as unread:', mailError)
        await mutate()
      }
    })
  }
  ...
  const handleDeleteMail = (mailId: string) => {
    startTransition(async () => {
      applyOptimistic({ type: 'delete', id: mailId })
      setSelectedMailId(null)

      try {
        await fetch(`/api/contact/${mailId}`, { method: 'DELETE' })
        await mutate()
      } catch (mailError) {
        console.error('Error deleting mail:', mailError)
        await mutate()
      }
    })
  }
  ...
  return (
    <div className="flex grow h-full min-h-0 rounded-xl shadow-xl overflow-hidden">
      <div className="sm:w-50 bg-[#F5F5F5] dark:bg-zinc-800 opacity-97 lg:w-75 w-full sidebar overflow-hidden shadow-inner flex flex-col min-h-0 rounded-l-[inherit] sm:rounded-r-none rounded-r-[inherit]">
              ...
              <div className="py-2">
                {optimisticMails.map((mail, index) => (
                  <Fragment key={mail._id}>
                    ...
                    >
                      <div className="block">
                        <div className="h-12.5 w-12.5 rounded-full relative">
                          <Image
                    ...
                    {optimisticMails.length > 1 &&
                      index < optimisticMails.length - 1 && <hr />}
                  </Fragment>
                ))}
              </div>
            </div>
      ...
      <div className="flex flex-col bg-white/95 dark:bg-zinc-800/95 flex-1 h-[inherit] panel overflow-hidden rounded-r-[inherit]">
        <div
          className={`border-b border-[#3C3C43]/36 dark:border-zinc-700 border-solid px-4 pt-2.5 pb-3 flex gap-x-4 h-23 flex-col justify-end`}
        >
          {selectedMail && (
