...
export default function Home() {
  ...
  return (
    <>
      <header className='flex justify-between items-center px-12 py-4 sm:px-4 sm:py-1.5 w-full xl:hidden'>
        <DisplayTime />
        <DeviceStatus />
      </header>
      ...
      <main className='py-9 sm:py-12 flex-grow'>
        <section className='grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-y-5 sm:gap-y-12 px-5 sm:gap-x-12 md:gap-x-16 lg:gap-x-28 sm:px-12 md:px-0 gap-x-7 xl:hidden'>
          {dock.map((item) => (
            <div
              key={item.name}
              className='grid place-items-center gap-y-1 w-full'
            >
              <Image alt={item.name} src={item.img} height={60} width={60} />
      ...
      <footer className='flex overflow-hidden gap-x-[30px] sm:gap-x-4 py-5 px-5 sm:px-7 sm:py-4 before:absolute before:content-[""] before:bg-[#BFBFBF70]/44 before:backdrop-blur-[50px] before:-z-10 bg-white/30 rounded-[40px] sm:rounded-[30px] before:content items-center justify-center my-3 sm:my-4 xl:hidden'>
        {dock.map((item) => (
          <Image
            key={item.name}
            alt={item.name}
